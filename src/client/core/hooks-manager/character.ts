...
import { Player } from "client/core/utility/utility";
...
export class CharacterAddController implements OnStart {
	public onStart(): void {
		...
		async function characterAdded(character: Model): Promise<void> {
			for (const listener of addListeners) listener.onCharacterAdd(<CharacterModel>character);
		}
		...
		Player.CharacterRemoving.Connect(async (model) => {
			for (const listener of removeListeners) listener.onCharacterRemove(<CharacterModel>model);
		});
	}
}
