...
import type { OnPlayerJoin } from "server/hook-managers/hooks";
...
export class CharacterAddService implements OnStart, OnPlayerJoin {
  ...
  public onPlayerJoin(player: Player): void {
    ...
    player.CharacterRemoving.Connect(async (model) => {
      for (const listener of this.removeListeners) listener.onCharacterRemove(<CharacterModel>model);
    });
  }

  private async characterAdded(character: Model): Promise<void> {
    for (const listener of this.addListeners) listener.onCharacterAdd(<CharacterModel>character);
  }
}
